...
void test_sdl_callback(void *userdata, Uint8 *stream, int len)
{
	...
	while(offs < len)
	{
		if(sound_queue_pos < SACKIT_TEST_AU_BUFFER)
		{
			int xlen = SACKIT_TEST_AU_BUFFER-sound_queue_pos;
			if(xlen > len-offs)
				xlen = len;
			...
			memcpy(sound_queue, nvbuf, SACKIT_TEST_AU_BUFFER*2);
			sound_queue_pos = 0;
			sound_ready = 1;
		}
	}
...
int main(int argc, char *argv[])
{
	...
	sackit_playback_t *sackit = sackit_playback_new(module);
	
	SDL_AudioSpec aspec;
	aspec.freq = 44100;
	aspec.format = AUDIO_S16SYS;
	aspec.channels = 1;
	aspec.samples = SACKIT_TEST_AU_BUFFER;
	aspec.callback = test_sdl_callback;
	sound_buf = calloc(1,SACKIT_TEST_AU_BUFFER*2);
	sound_queue = calloc(1,SACKIT_TEST_AU_BUFFER*2);
	...
	while(!quitflag)
	{
		...
		if(play_a_sound && sound_ready)
		{
			...
			int16_t *nvbuf = (int16_t *)sound_buf;
			memcpy(nvbuf, sackit->buf, SACKIT_TEST_AU_BUFFER*2);
			sound_ready = 0;
		}
